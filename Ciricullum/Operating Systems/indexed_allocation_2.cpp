...
void searchFile(){
    ...
    cout<<"FileName\tIndexedBlock\tAllocated Blocks\n";
    cout<<files[index].name<<"\t\t"<<files[index].idx->blockid<<"\t\t";
    files[index].idx->print();
}
...
void findBlock(){
    ...
    cin>>k;
    if(files[index].idx->blocks.size()<k) cout<<"File occupies less than "<<k<<" blocks!"<<endl;
    else cout<<"Block "<<k<<" of the given file is : "<<files[index].idx->blocks[k-1]<<endl;
    return;
}
